...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadData = async () => {
    try {
      ...
      // Delete orphaned transactions from DB asynchronously
      ...
      let finalState = reducer(initialState, {
        type: 'LOAD_STATE',
        payload: {
      ...
      // Apply offline queue to local state so it reflects offline actions
      try {
        const queuedActions = await getQueuedActions();
        for (const action of queuedActions) {
          // Re-apply the action to the local state
          finalState = reducer(finalState, action as any);
        }
      } catch (err) {
        console.error('Failed to apply offline queue to local state', err);
      }
  ...
  const dispatch = async (action: Action, isSyncingQueue = false) => {
    if (role === 'viewer') {
      console.warn('Viewers cannot perform mutations.');
      return;
    }
    ...
    try {
      ...
      switch (action.type) {
        case 'ADD_PAYMENT':
          processedAction = { ...action, payload: { ...action.payload, id: action.payload.id || crypto.randomUUID(), date: action.payload.date || new Date().toISOString() } };
          break;
        case 'ADD_STUDENT':
          processedAction = { ...action, payload: { ...action.payload, id: action.payload.id || crypto.randomUUID() } };
          break;
        case 'ADD_CATEGORY':
          processedAction = { ...action, payload: { ...action.payload, id: action.payload.id || crypto.randomUUID() } };
          break;
        case 'ADD_EVENT':
          ...
          break;
        case 'IMPORT_STUDENTS':
          processedAction = { ...action, payload: action.payload.map(s => ({ ...s, id: s.id || crypto.randomUUID() })) };
          break;
      }
